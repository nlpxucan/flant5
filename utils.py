...
def build_t5_finetune_optimizer(opt, model):
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=5e-5, eps=opt.adam_epsilon, betas=(0.9, 0.98))
...
def build_t5_optimizer(opt, model):
	optimizer = Adafactor(model.parameters(), scale_parameter=False, relative_step=False, warmup_init=False, lr=opt.learning_rate, weight_decay=1e-5)
	return optimizer

def build_adam_optimizer(opt, model):
